...
@app.route('/')
def index():
    #Dodanie pozyskanej bazy danych
    driver = flask.current_app.config['DRIVER']
    db = flask.current_app.config['DB']
    start_coords = start_location(driver, db)
...
def calculate_route():
    ...
    start, dist_to_start = find_nearest_node(driver, db, point1[1], point1[0])
    end, dist_to_end = find_nearest_node(driver, db, point2[1],point2[0])

    path1, time1 = aGwiazdka(driver, db, start, end, route_type)
    path2, time2 = path1, time1 #aGwiazdka(start, end, route_type, path1)
    ...
    route_coords1 = rebuild_route(driver, db, path1)
    route_coords2 = rebuild_route(driver, db, path2)
