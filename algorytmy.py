...
#### Algorytm Dijkstry -- nie używany
def dijkstra(start: Node, end: Node, route_type: str, prev_route = None):
    S = set()                           # odwiedzone
...
def cost(e: Edge, route_type: str, prev_route) :
    ...
    if prev_route and e in prev_route:  # przeszliśmy po tej krawędzi poprzednim razem
        n = len(prev_route) - 1             # indeks ostatniej krawędzi na liście poprzedniej trasy
        i = prev_route.index(e)             # indeks badanej krawędzi na liście poprzedniej trasy
        max_cm = 0.7                        # maksymalne obciążenie dodatkowe trasy
        cost_multiplier = bellcurve(n, max_cm, i)              
    v = e.speed / 3.6                       # zmiana jednostki prędkości km/h -> m/s
    time = e.length / v
    if route_type == "fastest":
